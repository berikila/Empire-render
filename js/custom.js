    ...
    jQuery(document).ready(function($) {;
	 ...
	 =======================================*/
        ...
        if ($('html.js.mutationobserver.cssanimations:not(.is-mobile) .stagger-word-animation').length) {
            ...
            TweenMax.staggerFrom(splitText.words, .5, {
                opacity: 0
            }, .1, allDone);
        ...
        if ($('.team-section').length) {
            ...
            function isInFlowState(date) {
                //Get Time and Day in Central Time
                let inFlowState = false;
                const flowDays = [1, 2, 3, 4, 5];
                dayOfWeek = date.getDay();
                if (flowDays.includes(dayOfWeek)) {
                    let hour = date.getHours();
                    let minutes = date.getMinutes();

                    // Monday 1-5pm
                    if (dayOfWeek == 1 && hour >= 13 && hour < 17) {
                        inFlowState = true;
                    }
                    // Tues, Wed, Thus 8:30-11am and 3:30-5:30pm
            ...
            const currentDate = changeTimeZone(new Date(), 'America/Chicago');

            if (isInFlowState(currentDate)) {
                $('.flow-state').show();
            } else {
                $('.flow-state').hide();
            }
            ...
            match: function() {
                ...
                if ($('body').hasClass('home')) {
                    ...
                    function homeListWorks() {
                        ...
                        var workWidth = 0;
                        $('.home__list-work > li').not('.view-all-card').each(function(index, el) {
                            workWidth += $(this).outerWidth();
                            // console.log('single workWidth: ' + $(this).outerWidth() + '  total workWidth: ' + workWidth);
                        });

                        containerMargin = $('.work-cards-carousel-section .container')
                            .css('margin-left');
                        if (containerMargin) {
                            containerMargin.replace('px', '');
                        }
        ...
        enquire.register('screen and (min-width: 992px)', {
            match: function() {
                if ($('body').hasClass('home')) {
                    ...
                    heroTimeline.add([
                        TweenMax.to('.slogan', 0.2, {
                            opacity: 0,
                            delay: 0.2
                        }),
                        TweenMax.to('.butn', 0.2, {
                            opacity: 0,
                            delay: 0.2
                        }),
                        TweenMax.to('.mouse', 0.2, {
                            opacity: 0,
                            delay: 0.2
                        }),
